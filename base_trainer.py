...
class BaseTrainer(object):
    ...
    def get_cpu_parameters(self, named = True):
    ...
    def one_epoch(self):
        ...
        # Training
        training_data_func = self.buffer.generate_episodic_data if self.training_params['is_episodic'] else self.buffer.generate_transition_data
        for batch in training_data_func(self.training_params['batch_size'], self.device):
    ...
    def visualize_gradient_flow(self):
        ...
        training_data_func = self.buffer.generate_episodic_data if self.training_params['is_episodic'] else self.buffer.generate_transition_data
        batch = next(training_data_func(self.training_params['batch_size'], self.device))
        self.train(batch)
    
        plot_grad_flow(self.get_cpu_named_parameters())
