...
def augmented_random_search(env, num_timesteps, α, ν, N, b, p, n, num_updates, result_filename, resume=False):
    ...
    for j in range(num_updates):
        ...
        avg_curr_rollout_x = np.mean(curr_rollout_x, 0)
        M_prev = Σ * last_nx
        M_curr = np.std(curr_rollout_x, 0)**2 * len(curr_rollout_x)
        M2 = M_prev + M_curr + (avg_curr_rollout_x - μ) ** 2 * last_nx * len(curr_rollout_x) / nx
        Σ = M2 / nx
        μ = (μ * last_nx + avg_curr_rollout_x * len(curr_rollout_x)) / nx
